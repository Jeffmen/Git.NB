...
import com.example.gitnb.module.viewholder.NotificationViewHolder;
...
public class NotificationListAdapter extends RecyclerView.Adapter<ViewHolder>{
	...
	public Notification getItem(int position) {
		...
		return notifications == null ? null : notifications.get(position-(isShowSearch?1:0));
	}
    ...
    public void update(ArrayList<Notification> data){
		isShowLoadMore = true;
    ...
    public void insertAtBack(ArrayList<Notification> data){
    	if(data == null || data.size()<PAGE_COUNT){
    		isShowLoadMore = false;
    	}
    	else{
	...
	public int getItemCount() {
		...
		if(notifications == null){
			...
			return notifications.size() + orther;
		}
	}
	...
	public ViewHolder onCreateViewHolder(ViewGroup viewgroup, int viewType) {
		if(viewType == TYPE_FOOTER_VIEW){
			...
			View v = mInflater.inflate(R.layout.notification_list_item,viewgroup,false);
			return new NotificationView(v);
		}
	}
	...
	public void onBindViewHolder(ViewHolder vh, int position) {		
		...
		case TYPE_NOMAL_VIEW:
			NotificationView viewHolder = (NotificationView) vh;
			Notification item = getItem(position);
            switch(item.subject.type) {
                case Issue:
					viewHolder.notification_type.setText("{fe-issue}");
                    break;
                case Commit:
					viewHolder.notification_type.setText("{fe-commit}");
                    break;
                case Release:
					viewHolder.notification_type.setText("{fe-release}");
                    break;
                case PullRequest:
					viewHolder.notification_type.setText("{fe-request}");
                    break;
            }
            viewHolder.repos_name.setText(item.repository.getName());
			viewHolder.notification_title.setText(item.subject.title);
			viewHolder.created_date.setText(Utils.getTimeFromNow(item.updated_at));
            break;
	...
	private class NotificationView extends NotificationViewHolder implements View.OnClickListener{
